...
def main() -> None:
    ...
    if choice == 'the president':
        ...
        """
         Allows the user to see all books from each selection.
         But it doesn't need the president to add the books in each selection first
        """
        goncourt = Goncourt()
        ...
        if password != "goncourt2025":
            ...
            return
        all_juries_from_first = []
        voted_books = []
        while len(all_juries_from_first) < 10:
            print("--------------------------------")
            ...
            if not any(j.__str__() == jury_name for j in goncourt.get_all_juries()):
                ...
                continue
            if any(j == jury_name for j in all_juries_from_first):
                print("This jury member has already voted. Please choose another one.")
                continue
            print("--------------------------------")
            print(f"Welcome {jury_name}!")
            print("You can now vote for your favorite book from the first selection.")
            print("--------------------------------")
            while True:
            ...
            all_juries_from_first.append(jury_name)

        all_juries_from_second = []
        while len(all_juries_from_second) < 10:
            print("--------------------------------")
            for jury in goncourt.get_all_juries():
                print(f"Jury Member: {jury}")
            jury_name = input("Choose your jury member from the list above by entering his full name: ").strip()
            if not any(j.__str__() == jury_name for j in goncourt.get_all_juries()):
                print("This jury member does not exist. Please choose a valid jury member from the list.")
                continue
            if any(j == jury_name for j in all_juries_from_second):
                print("This jury member has already voted. Please choose another one.")
                continue
            print("--------------------------------")
            print(f"Welcome {jury_name}!")
            print("You can now vote for your favorite book from the second selection.")
            print("--------------------------------")
            while True:
                choice = input("Do you want to vote for a book? (yes/no):").strip().lower()
                if choice == "yes":
                    print("Here are the books from the second selection:")
                    print("--------------------------------")
                    second_selection_books = goncourt.get_books_from_selection("second")
                    for book in second_selection_books:
                        print(book.__str__())
                        print("\n")
                    vote = int(input("Enter the book ID you want to vote for: "))
                    voted_book = None
